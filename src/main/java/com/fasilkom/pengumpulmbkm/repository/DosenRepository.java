...
public interface DosenRepository extends JpaRepository<Dosen, String> {
    ...
    Dosen findDosenByDosenId(@Param("dosen_id") String dosenId);
    @Query(value = "SELECT d.dosen_id, u.user_id, u.nama_lengkap, u.email, u.no_hp " +
            "from dosen d join users u on u.user_id=d.user_id where d.user_id=:user_id", nativeQuery = true)
    Dosen findDosenByUserId(@Param("user_id") String userId);
    ...
    void deleteDosenByDosenId(
            @Param("dosen_id") String dosenId);
    boolean existsDosenByDosenId(String dosenId);
    @Query(value = "SELECT count(*) > 0 FROM dosen WHERE user_id=:user_id", nativeQuery = true)
    boolean existsDosenByUserId(@Param("user_id") String userId);
}
